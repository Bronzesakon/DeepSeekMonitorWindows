  ...
  <div ref="rootEl" :class="[collapsed ? 'px-5 pt-2 glass-root' : 'p-4 glass-root']" @mouseenter="onTitleEnter" @mouseleave="onRootLeave">
    <div data-tauri-drag-region :class="collapsed ? 'relative flex items-center justify-center mb-2' : 'title-bar mb-2'" @mouseenter="onTitleEnter" @mousedown="onTitleDown">
      ...
      <span v-if="collapsed" class="inline-flex items-center text-sm font-semibold leading-none">
        <template v-if="currentStore.hasPlatformSession">
          <template v-if="activeProvider === 'mimo' && mimoSettings.paymentMode === 'plan'">
            <span class="text-sm text-gray-600 dark:text-gray-300">套餐</span><span class="text-sm text-orange-500 dark:text-orange-400 ml-1">{{ (mimoStore.planUsagePercent ?? 0).toFixed(1) }}%</span>
          </template>
          <template v-else>
            <span class="text-sm text-gray-600 dark:text-gray-300">今日费用</span><span class="text-sm text-orange-500 dark:text-orange-400 ml-1">¥{{ currentStore.currentDayCost.toFixed(2) }}</span>
          </template>
      ...
      <div v-if="!collapsed" class="title-actions">
        <button v-if="currentStore.hasPlatformSession" aria-label="刷新" class="title-btn refresh-btn" @click="currentStore.refresh()"><RefreshIcon /></button>
    ...
    <div v-if="currentStore.isFirstLaunch || !currentStore.hasPlatformSession" class="min-h-[500px]">
      <OnboardingView />
    </div>
    <div v-else class="space-y-3">
      <ErrorBanner :error="currentStore.errorMessage" :warning="currentStore.warningMessage" :showSettings="true" />
    ...
    <div v-else class="space-y-3">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="glass-card p-5">
          <div class="flex items-center gap-2 mb-1"><span class="w-2 h-2 rounded-full inline-block" style="background: #8B5CF6"></span><span class="text-xs font-semibold tracking-wider text-gray-500 dark:text-gray-400">V2.5 Pro 今日消耗</span></div>
          <div class="text-xl font-bold text-gray-800 dark:text-gray-100">{{ mimoStore.currentDayProTokens.toLocaleString() }}</div>
        </div>
        <div class="glass-card p-5">
          <div class="flex items-center gap-2 mb-1"><span class="w-2 h-2 rounded-full inline-block" style="background: #0EA5E9"></span><span class="text-xs font-semibold tracking-wider text-gray-500 dark:text-gray-400">V2.5 今日消耗</span></div>
          <div class="text-xl font-bold text-gray-800 dark:text-gray-100">{{ mimoStore.currentDayFlashTokens.toLocaleString() }}</div>
        </div>
      </div>
      <MimoTokenDetail />
      <MimoTrendChart />
...
// Reset mouse/drag state when provider switches (prevents stuck collapsed state)
...
onMounted(async () => {
  // Load settings FIRST so payment mode is correct before data refresh
  await Promise.all([
  ...
  listen("ds-trigger-refresh", (event: { payload: any }) => {
    if (event.payload && typeof event.payload === 'object' && 'has_platform_session' in event.payload) dsStore.applyData(event.payload);
    else dsStore.refresh();
  });

  // MiMo events
  listen("mimo-login-complete", () => { invoke("mimo_broadcast_refresh"); });
  listen("mimo-trigger-refresh", (event: { payload: any }) => {
    if (event.payload && typeof event.payload === 'object' && 'has_platform_session' in event.payload) mimoStore.applyData(event.payload);
    else mimoStore.refresh();
  });
  ...
  listen("edge-snap-setting-changed", (event: { payload: boolean }) => {
    ...
    if (event.payload) { getCurrentWindow().outerPosition().then(async pos => { const mt = (await monitorFromPoint(pos.x, pos.y))?.position.y ?? 0; if (pos.y - mt <= 10 && !collapsed.value) { docked.value = true; collapsed.value = true; invoke("hide_trend_detail"); } }).catch(() => {}); }
    else if (docked.value) { docked.value = false; if (collapsed.value) collapsed.value = false; }
  });

  const [_snap, u1, u2] = await Promise.all([
    settings.loadEdgeSnapEnabled(),
    getCurrentWindow().listen<{ label: string }>("edge-snap-triggered", (event) => { if (event.payload.label !== currentLabel) return; clearDragGuard(); if (settings.edgeSnapEnabled) { docked.value = true; collapsed.value = true; invoke("hide_trend_detail"); } }),
  ...
  mainWin.onMoved(() => { invoke("reposition_trend_detail"); });
...
function closePanel() { invoke("hide_trend_detail"); invoke("hide_main_window"); }
</script>
