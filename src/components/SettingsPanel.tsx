...
export function SettingsPanel({ provider, onProviderChange, onBack, onUsageLoaded, onUsageCleared, onRefreshIntervalChanged, onAutoRefreshChanged }: {
  ...
  const categories = [
    ...
    { key: "general", icon: <Settings size={15} />, label: '通用' },
    { key: "display", icon: <Monitor size={15} />, label: '显示' },
    { key: "notify", icon: <Bell size={15} />, label: '通知' },
  ...
  const displayContent = (
    <>
      <SettingsSection icon={<Palette size={15} />} title='主题'>
        <p>选择应用的外观主题。</p>
  ...
  const notifyContent = (
    <SettingsSection icon={<Bell size={15} />} title='通知'>
      <Toggle label='余额不足时发送 Windows 通知' checked={lowBalanceNotify} onChange={saveLowBalanceNotify} />
      <p>当 API 余额低于设定阈值时，通过 Windows 通知提醒。</p>
      {lowBalanceNotify && (
        <>
